...
export default function Cart() {
  ...
  function handleQuantityChange(e, id, color) {
    const _cartItems = cartItems.map((el) => {
      if (el.id === id && el.color === color && e.target.value !== "") {
        ...
        alert(
          "Bạn vui lòng đặt từ 1 - " + el.max_color_quantity + " sản phẩm."
        );
      }
      return el;
    });
                      ...
                      <div className="blk-qty d-flex justify-content-center align-items-center">
                        ...
                        <input
                          ...
                          value={
                            cartItems.find(
                              (el) =>
                                el.id === item.id && el.color === item.color
                            ).cart_quantity
                          }
                          onChange={(e) =>
                            handleQuantityChange(e, item.id, item.color)
                          }
                        />
                        <div
                          data-label="cart"
